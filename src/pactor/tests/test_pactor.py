...
class PactorTests(TestCase):
    def test_actor_has_proper_methods(self):
        """ Validates that an @actor has the correct methods """
        with actor_test_context(SampleActor) as context:
            self.assertTrue(callable(context.actor.one))
            self.assertTrue(callable(context.actor.two))
            self.assertTrue(callable(context.actor.three))
